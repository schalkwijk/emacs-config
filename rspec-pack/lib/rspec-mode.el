...
(defun rspec-spec-file-for (a-file-name)
  ...
  (if (rspec-spec-file-p a-file-name)
      a-file-name
    (let ((replace-regex (if  (rspec-spec-directory-has-lib? a-file-name) ;;(and (rspec-target-lib-file-p a-file-name) (rspec-spec-directory-has-lib? a-file-name))
                             "^\\.\\./"
                           "^\\.\\./[^/]+/"))
